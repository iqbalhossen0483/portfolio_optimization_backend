...
- Time-series normalization of ESG would mix the "good ESG period" vs "bad ESG period" signal — which is not what we want. We want to know: is this asset better or worse than its peers *today*.
...
state[t] = [
    ...
    rsi_norm(t, 1..N),      ← N values: normalized RSI (14-period, from XLSX)
...
**Snapshot key:** Every published message is also stored in `training:snapshot:{job_id}` (TTL 1 hour). When a browser connects *after* training has finished, it immediately receives the last known state instead of seeing nothing.
